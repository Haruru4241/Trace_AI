...
public class Grid7 : MonoBehaviour
{
    ...
    void CreateGrid()
    {
        grid7 = new Node7[gridSizeX, gridSizeY];
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int y = 0; y < gridSizeY; y++)
            {
                ...
                grid7[x, y] = new Node7(worldPoint, x, y, movementPenalty);
            }
        }
    }

    public List<Node7> GetNeighbours(Node7 node)
    {
        List<Node7> neighbours = new List<Node7>();
        ...
        for (int x = -1; x <= 1; x++)
        {
            for (int y = -1; y <= 1; y++)
            {
                ...
                {
                    neighbours.Add(grid7[checkX, checkY]);
                }
            }
        }
    ...
    public Node7 NodeFromWorldPoint(Vector3 worldPosition)
    {
