...
public abstract class MapGenerator : MonoBehaviour
{
    ...
    protected void DrawMap(List<BlockType>[,] mapBlocks, Transform parent)
    {
        for (int x = 0; x < mapSize; x++)
        {
            for (int z = 0; z < mapSize; z++)
            {
                ...
                {
                    prefabToInstantiate = hallwayPrefab;
                }
                ...
                {
                    GameObject instantiatedObject = Instantiate(prefabToInstantiate, new Vector3(x, 0, z), Quaternion.identity);
    ...
    protected bool IsTouchingWall(Vector2Int position)
    {
        ...
        {
            if (mapBlocks[position.x, position.y].Contains(BlockType.Wall)
